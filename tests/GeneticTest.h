...
namespace GeneticTest {
    ...
    TEST(GeneticTest, FinishNetwork_Canon){
        // check canonical exaples
        for(const Network& net : CANON_NETS) {
            Genetic solv(net, ALL_COSTS, CONS, 0);
            EXPECT_TRUE(
                CONS(
                    solv.finishNetwork(
                        solv.getNet()
    ...
    TEST(GeneticTest, FinishNetwork_Extra){
    ...
    TEST(GeneticTest, FinishNetwork_Random_GridSize){
    ...
    TEST(GeneticTest, FinishNetwork_Random_Add_Facts){
    ...
    // spliceRoutes
    /////////////////
    TEST(GeneticTest, SpliceRoutes_Canon){
    ...
    TEST(GeneticTest, SpliceRoutes_Extra){
        FactoryKey BASE_LINK(314251, 1242121);
        // check canonical exaples
        for(Network net : CANON_NETS) {
            // add extra
    ...
    TEST(GeneticTest, SpliceRoutes_Random){
        FactoryKey BASE_LINK(314251, 1242121);
    ...
    TEST(GeneticTest, Solve_Canon_Valid){
        // check canonical exaples
        for(const Network& net : CANON_NETS) {
            Genetic solv(net, ALL_COSTS, CONS, NUM_ITERS);
            EXPECT_TRUE(CONS(solv.solve()));
        }
    }
    TEST(GeneticTest, Solve_Canon_Improved){
        // check canonical exaples
        for(const Network& net : CANON_NETS) {
            Genetic solv(net, ALL_COSTS, CONS, NUM_ITERS);
    ...
    /*
    //These tests run long and fail cuz the genetic algorithm is bad

    #include <iostream>
    TEST(GeneticTest, Solve_Random_GridSize){
        // check random networks with an increasing grid size and a constant number of factories
        const int numFacts = 30;
        for(int gridSize = 5; gridSize < MAX_LOC_COORD; gridSize += 10){
            size_t s = rand();
    ...
    TEST(GeneticTest, Solve_Random_Add_Facts){
        // check random networks with increasing number of factories
